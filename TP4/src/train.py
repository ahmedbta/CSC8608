...
def main() -> None:
    ...
    data = load_cora()
    pyg_data = data.pyg_data.to(device) if hasattr(data, 'pyg_data') else None
    x = pyg_data.x if pyg_data is not None else data.x.to(device)
    y = pyg_data.y if pyg_data is not None else data.y.to(device)
    edge_index = pyg_data.edge_index if pyg_data is not None else data.edge_index.to(device)
    train_mask = pyg_data.train_mask if pyg_data is not None else data.train_mask.to(device)
    val_mask = pyg_data.val_mask if pyg_data is not None else data.val_mask.to(device)
    test_mask = pyg_data.test_mask if pyg_data is not None else data.test_mask.to(device)
    ...
    if args.model == "sage":
        ...
        try:
            ...
            # test that it actually works (requires pyg-lib or torch-sparse)
            _ = next(iter(train_loader))
            print("sampling: NeighborLoader OK (mini-batch)")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if args.model in ["mlp", "gcn"] or (args.model == "sage" and train_loader is None):
            with Timer() as t:
                if args.model == "mlp":
                    logits = model(x)
                else:
