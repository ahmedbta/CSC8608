...
def main():
    ...
    python_bin = r"C:/Users/ahmed/Desktop/CSC8608/venv/Scripts/python.exe"
    # 1) VAD
    run(f'"{python_bin}" TP3/vad_segment.py')
    vad = load_json("TP3/outputs/vad_segments_call_01.json")

    # 2) ASR
    run(f'"{python_bin}" TP3/asr_whisper.py')
    asr = load_json("TP3/outputs/asr_call_01.json")

    # 3) Analytics
    run(f'"{python_bin}" TP3/callcenter_analytics.py')
    ...
    tts_done = False
    if tts_path.exists():
        run(f'"{python_bin}" TP3/tts_reply.py')
        tts_done = True
